...
class Players(db.Model):
    ...
    events = db.Relationship(
        "Events", back_populates="player", cascade="all, delete")
    comments = db.Relationship(
        "Comments", back_populates="player", cascade="all, delete")
...
class PlayerSchema(ma.Schema):
    ...
    events = fields.List(fields.Nested("EventSchema", only=[
                         "description", "date", "duration"]))
    comments = fields.List(fields.Nested(
        "CommentSchema", only=["id", "message"]))
    ...
    class Meta:
        fields = ("id", "name", "date", "role",
                  "game", "comments", "events", "user")
