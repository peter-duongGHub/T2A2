...
class Users(db.Model):
    ...
    games = db.Relationship(
        "Games", back_populates="user", cascade="all, delete")
    players = db.Relationship(
        "Players", back_populates="user", cascade="all, delete")
...
class UserSchema(ma.Schema):
    ...
    games = fields.List(fields.Nested(
        "GameSchema", only=["name", "description"]))

    players = fields.List(fields.Nested(
        "PlayerSchema", only=["name", "role", "date"]))
    # Validation of attributes, restricting user input to certain conditions
    username = fields.String(required=True, validate=Regexp(
        "^[a-zA-Z]{1,50}$", error="Name must only contain characters A-Z, between 1 to 50 characters."))
